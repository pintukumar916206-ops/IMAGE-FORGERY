...
class ForgeryDetector:
    ...
    def detect_file(self, file_path: str, ml_result: dict = None) -> dict:
        try:
            ...
            if ml_score > 70:
                flags += 1
                reasons.append(f"AI/ML model identifies high manipulation probability ({ml_score}%).")

            is_forged = flags >= 2 or ml_score > 80
            
            if flags == 0 and ml_score < 30:
                confidence = 95
                ...
                confidence = 85
                reasons.append("Multiple forensic anomalies confirm intentional manipulation.")
            else:
                confidence = 50
